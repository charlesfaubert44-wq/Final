...
def render_dashboard():
    ...
    with col3:
        ...
        <div class="stat-card" style="position: relative; overflow: hidden;">
            <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); font-size: 8rem; opacity: 0.08; z-index: 0;">🏔️</div>
    ...
    with chart_col1:
        st.caption("CASE STATUS DISTRIBUTION")
        ...
        if status_data:
            ...
            # Use Streamlit's built-in bar chart with WSCC colors - SMALLER
            st.bar_chart(status_df.set_index('Status'), color='#2c5282', height=180)
        else:
            st.info("No data available")

    with chart_col2:
        st.caption("TERRITORY COMPARISON")
        ...
        if territory_data:
            ...
            # Use bar chart - SMALLER
            st.bar_chart(territory_df.set_index('Territory'), color='#4299e1', height=180)
        else:
            st.info("No data available")
    ...
    with chart_col3:
        ...
        if priority_data:
            ...
            st.bar_chart(priority_df.set_index('Priority')['Count'], color='#f59e0b', height=180)
        else:
            st.info("No data available")

    with chart_col4:
        st.caption("INVESTIGATOR WORKLOAD ANALYSIS")
        ...
        if officer_workload:
            ...
            st.bar_chart(workload_df.set_index('Officer'), color='#10b981', height=180)
        else:
    ...
    st.caption("TEMPORAL CASE ANALYSIS")
    if all_cases:
        ...
        if case_dates:
            ...
            st.line_chart(timeline_df.set_index('Month'), color='#2c5282', height=200)
        else:
